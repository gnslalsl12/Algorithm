...
/*
 * 왼쪽 위부터 오른쪽 아래로 탐색하다가
 * 섬 만나면 섬 번호 기입
 * 바다 만나면 해당 바다가 어떤 섬으로 둘러싸여있는지 확인
 * -> 범위 밖으로 나가면 둘러싸인 바다 X
 * -> 번호가 정해진 섬으로 둘러싸여있으면 해당 바다를 모두 그 섬 번호로 표기
 * 	<- (번호가 정해지지 않은 섬으로 둘러싸일 일은 없음. 탐색 방향이 한 방향이기 때문)
 * 이후 각 섬들의 첫 위치들만 보면서 8방 확인했을 때
 * -> 바다를 만나면 해당 섬은 다른 섬으로 둘러싸여있지 않음 => 모섬은 자기 자신
 * -> 어떤 섬을 만나면 그 섬으로 둘러싸여있음 확인 => 모섬은 그 둘러싸고있는 섬
 * 마지막으로 각 섬들의 모섬을 확인하면서 크루스칼 처럼 모섬에게 높이를 +1씩
 * => 완료
*/
