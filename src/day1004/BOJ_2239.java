...
public class BOJ_2239 { // BitMasking
	static int[][] maps = new int[9][9];	//맵
	static int[] rowBM = new int[9];		//행 BM 리스트
	...
	public static void main(String[] args) throws IOException {
		...
		// BM 매핑 완료
		while (!dirXY.isEmpty()) { // 채워야하는 곳들 하나씩 봤을 때
			int[] temp = dirXY.poll();
			int x = temp[0];
			int y = temp[1];
			int k = temp[2];
			...
			int count = 0;
			for (int i = 1; i <= 9; i++) {
				if ((rBM & (1 << i)) == 0 && (cBM & (1 << i)) == 0 && (bBM & (1 << i)) == 0) { // 행에도 없고 열에도 없고 블록에도 없는
																								// 숫자
	...
	private static void DFS(int count) {
		...
		if (maps[x][y] != 0) {
			...
			for (int i = 1; i <= 9; i++) {
				if ((rowBM[x] & (1 << i)) == 0 && (colBM[y] & (1 << i)) == 0 && (blockBM[getK(x, y)] & (1 << i)) == 0) {	//선택되지 않은 숫자야!
					rowBM[x] |= 1 << i;
					colBM[y] |= 1 << i;
					...
					if (done)
						return;
					maps[x][y] = 0;	// i로 보내봤는데 다음 경로에서 해당 숫자를 또 써야해 => 재귀함수 복귀 및 i 다음 숫자로 재시도
					rowBM[x] &= ~(1 << i);
					colBM[y] &= ~(1 << i);
					blockBM[getK(x, y)] &= ~(1 << i);
				}
