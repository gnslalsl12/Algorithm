...
public class BOJ_10800_re_re {
	...
	public static void main(String[] args) throws IOException {
		...
		long[] ColorizeSizeSum = new long[200001];	//color값 index화 해서 저장한 color별 사이즈 합
		long[] SizerizeSum = new long [2001];		//같은 사이즈를 가진 볼 제외해주기 위해 저장한 사이즈별 합계값
		
		long TotalSumSize = 0;
		long SameSizeSameColor = 0;					//색이 같고 사이즈가 같은 놈을 빼주기 위한 합게깞
		...
		while(!PlayerList.isEmpty()) {
			...
			ColorizeSizeSum[temp.Color] += (long)temp.Size;	//색깔별 사이즈 합계
			SizerizeSum[temp.Size] += (long)temp.Size;		//사이즈별 사이즈 합계
			
			TotalSumSize += (long)temp.Size;		//지금까지 모든 사이즈 합계
			
			long SumToPush = TotalSumSize - SizerizeSum[temp.Size] - ColorizeSizeSum[temp.Color] + SameSizeSameColor;
	...
	private static class PlayerWithOnlySize implements Comparable<PlayerWithOnlySize>{
		int Nth;
		long SizeSum;
		public PlayerWithOnlySize(int nth, long sizeSum) {
			super();
			Nth = nth;
			SizeSum = sizeSum;
		}
