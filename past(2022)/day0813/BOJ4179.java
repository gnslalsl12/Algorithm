...
//		System.out.println(R + C);
		...
		System.out.println("불 끝");
		DFSforJihoon(JihoonDir);
		
		
		/*for(int [] t : PassedJHway) {
			for(int tt : t) {
				System.out.printf("%d ", tt);
			}
			System.out.println();
	...
	static void BFSforFire(dirXY firexy) {
		...
		while(!BFSqueueF.isEmpty()) {	
			...
			for(int dd = 0; dd < 4; dd++) {
				...
				if(PassedFireway[nextx][nexty] == 0 || PassedFireway[nextx][nexty] > PassedFireway[poppedXY.x][poppedXY.y]) {	// 다음 경로의 발자국이 0이 아니면서 나보다 작을 때 = continue;
...
//					System.out.println("성공 : " + nextx + ", " + nexty);
